...
export function registerResources(server: McpServer): void {
  ...
  server.resource("all-markdown", "markdown://all", async () => {
    const fileList = Array.from(markdownFiles.entries()).map(
      ([file, content]) => ({
        file,
        content,
        uri: `markdown://${file}`,
 ...
 * MCPリソースを登録
 */
export function registerResourcesNew(server: McpServer): void {
  // 全マークダウンファイルの一覧
 ...
 * 個別ファイル用のリソースを追加
 */
export function addResourcesForFiles(server: McpServer): void {
 ...
 * 個別ファイル用のリソースを追加
 */
export function addResourcesForFilesNew(server: McpServer): void {
  for (const file of markdownFiles.keys()) {
    // Remove leading "../" and use the full path as resource name
    const resourceName = file.replace(/^\.\.\//, "");
