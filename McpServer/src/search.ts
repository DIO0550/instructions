import { PromptMetadata, SearchResult, TechStack } from "./types.js";
 ...
 * コンテキストから技術スタックを検出
 */
function detectTechStack(context: string): TechStack {
  return {
    ...
    implementation:
      /\b(implement|implementation|develop|code|function)\b/i.test(context),
  };
}
 ...
 * 関連プロンプト検索
 */
export function getRelevantPrompts(context: string): PromptMetadata[] {
  const relevantPrompts: PromptMetadata[] = [];
  ...
  // 重複除去
  const uniquePrompts = relevantPrompts.filter(
    (prompt, index, self) =>
 ...
 * ファイル名でプロンプトを検索
 */
export function findPromptByFilename(filename: string): string | null {
  let actualFile = filename;
  if (!actualFile.endsWith(".md")) {
    actualFile = `${actualFile}.md`;
  }

  const matchingFile = Array.from(promptMetadata.keys()).find(
    (file) => file === actualFile || file.endsWith(`/${actualFile}`)
  );
