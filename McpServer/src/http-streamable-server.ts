...
import {
  ...
  registerResourcesNew,
  addResourcesForFilesNew,
} from "./resources.js";
import { registerPrompts, registerPromptsNew } from "./prompts.js";
import { registerTools, registerToolsNew } from "./tools.js";
 ...
 */
async function runStreamableHttpServer(): Promise<void> {
  try {
    // マークダウンファイルの読み込み
      ...
      try {
        ...
        if (!sessionId) {
          ...
          const transport = new StreamableHTTPServerTransport({
            ...
            onsessioninitialized: (sessionId: string) => {
              console.error(
                `[MCP] SSE session initialized with ID: ${sessionId}`
              );
              transports.set(sessionId, transport);
              servers.set(sessionId, server);
            },
    ...
    app.post("/messages", async (req: Request, res: Response) => {
      ...
      if (!sessionId || !sseTransports.has(sessionId)) {
        ...
        console.error(
          "[SSE Messages] Available sessions:",
    ...
    app.get("/sse", async (req: Request, res: Response) => {
      ...
      try {
        ...
        res.write(
          `data: {"sessionId":"${newSessionId}","type":"connection","status":"connected"}\n\n`
        );
    ...
    // ポート設定
    const port = parseInt(
      process.env.HTTP_PORT || process.env.PORT || "3002",
      10
    );
    ...
    const httpServer = app.listen(port, "0.0.0.0", () => {
      ...
      console.error(
        `claude mcp add --transport http prompt-server http://localhost:${port}/mcp`
      );
      console.error(
        `claude mcp add --transport sse prompt-server http://localhost:${port}/sse`
      );
      console.error(
        `claude mcp add --transport sse prompt-server http://localhost:${port}/mcp`
      );
      console.error(`==========================================\n`);
    });
