 ...
 * MCPツールを登録
 */
export function registerToolsNew(server) {
    // implementation-workflow.prompt.md専用取得ツール
    server.registerTool("get_implementation_workflow", {
        title: "Get Implementation Workflow",
        description: "実装フローとGitワークフローのガイドライン（implementation-workflow.prompt.md）を取得します",
        inputSchema: {},
    }, async () => {
        const workflowFile = "implementation-workflow.prompt.md";
        const matchingFile = findPromptByFilename(workflowFile);
    ...
    server.registerTool("search_prompts", {
        ...
        description: "キーワードでプロンプトを検索し、関連するプロンプトファイルを取得します",
        inputSchema: {
            query: z
                .string()
                .describe("検索キーワード（技術用語、カテゴリ、機能名など）"),
    ...
    // コンテキストに基づいて関連プロンプトを自動取得
    server.registerTool("get_relevant_prompts", {
        title: "Get Relevant Prompts",
        description: "現在のコンテキストに基づいて関連するプロンプトを自動的に取得します",
        inputSchema: {
            context: z
                .string()
                .describe("現在の作業コンテキスト（技術スタック、作業内容、問題点など）"),
    ...
    // AIが必要と判断したプロンプトを自動取得
    server.registerTool("auto_get_prompt", {
