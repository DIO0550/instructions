...
class MCPTCPBridge {
    ...
    server;
    connections = new Map();
    constructor(port = 3000, mcpExecutable = "server.js") {
        this.port = port;
    ...
    handleConnection(socket) {
        ...
        console.log(`[${new Date().toISOString()}] Client connected from ${clientId}`);
        ...
        // ソケットイベントハンドラ
        socket.on("error", (err) => {
            console.error(`[${new Date().toISOString()}] Socket error for ${clientId}:`, err);
        ...
        mcpProcess.on("exit", (code) => {
            console.log(`[${new Date().toISOString()}] MCP process for ${clientId} exited with code ${code}`);
